...
class TestPositive:
    def test_step1(self, make_folders, clear_folders, make_files):  # фикстуры передаем как аргументы
        # test1
        res1 = checkout(f"cd {data.get('folder_in')}; 7z a {data.get('folder_out')}/arx", "Everything is Ok")
        res2 = checkout(f"ls {data.get('folder_out')}", "arx.7z")
        print(f'res1={res1}')
        print(res2)
    ...
    def test_step2(self, clear_folders, make_files):
        ...
        assert all(res), "test2 FAIL"  # пройдет, если все будут успешны
    ...
    def test_step5(self, clear_folders, make_files):
        ...
        res.append(checkout("cd {}; 7z a {}/arx2".format(data['folder_in'], data['folder_out']), "Everything is Ok"))
        for item in make_files:
            res.append(checkout("cd {}; 7z l arx2.7z".format(data['folder_out'], data['folder_ext']), item))
